...
public class Challenge22
{
    ...
    {
        var graph = Graph.ImplicitWeighted<State, int>(c => GetAdjacent(c, false));
    ...
    {
        var graph = Graph.ImplicitWeighted<State, int>(c => GetAdjacent(c, true));
        var (_, minSpent) = graph.Dijkstra().ShortestPath(new State(50, 58, 500, 0, 0, 0, 0), IsFinished);
        return minSpent.ToString();
    }
    ...
    private IEnumerable<WeightedEdge<State, int>> GetAdjacent(State current, bool hardMode)
    {
        if (hardMode)
        ...
        if (mana >= 53)
        {
            ...
            yield return new WeightedEdge<State, int>(current, nextState, Weight(current, nextState));
        }
